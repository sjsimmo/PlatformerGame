...
void graphics::init(const settings &Settings)
{
    ...
    window = SDL_CreateWindow("Epic Platformer Game - The Third Attempt", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, Settings.window_width, Settings.window_height, SDL_WINDOW_SHOWN);
    if(window == NULL) 
    {
...
void graphics::load_textures(const map &Map)
{
...
void graphics::update_layer(int layer, const map &Map)
{
    //Clear layer
